...
class Broadcast3000Experiment:
    def __init__(self):
        self.results_dir = "broadcast_3000_results"
        os.makedirs(self.results_dir, exist_ok=True)

        self.experiments = [
            {"n": 3000, "density": 0.01, "block_size": 3000},
            {"n": 3000, "density": 0.1,  "block_size": 3000},
        ]
    ...
    def run_experiment(self):
        ...
        print(f"{'='*80}")
        print(f"🚀 Broadcast-Only 3000x3000 测试")
        ...
        try:
            for i, exp in enumerate(self.experiments, 1):
                ...
                exp_id = f"Broadcast3000_Exp{i}_{n}x{n}_D{int(density*100)}"

                print(f"\n▶️  [{i}/{len(self.experiments)}] 正在运行: {n}x{n} | 密度 {density}")

                conf = (SparkConf()
                    .setAppName(f"Broadcast3000_{exp_id}")
    ...
    def save_and_plot(self, results):
        ...
        csv_path = os.path.join(self.results_dir, "broadcast_3000_metrics.csv")
        ...
        plt.xlabel('矩阵密度')
        plt.ylabel('时间 (秒)')
        plt.title('Broadcast-Only 3000x3000 矩阵性能分析')
        ...
        img_path = os.path.join(self.results_dir, "broadcast_3000_performance.png")
        plt.savefig(img_path)
        print(f"📊 图表已生成: {img_path}")

        print("\n" + "="*80)
        print("🔍 Broadcast-Only 3000x3000 实验分析总结:")
        print("="*80)
...
if __name__ == "__main__":
    print("="*80)
    print("🚀 开始运行 Broadcast-Only 3000x3000 测试")
    print("⚠️  注意：稠密矩阵可能需要较多内存")
    print("="*80)

    exp = Broadcast3000Experiment()
    exp.run_experiment()
